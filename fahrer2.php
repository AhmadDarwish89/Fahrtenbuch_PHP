...
if(isset($_POST['delete_fahrt'])) { 
 $fahrer_id = $_POST['driver'];
$pdo = new PDO('mysql:host=localhost;dbname=fahrtenbuch', 'root', '');


$stmt = $pdo->prepare('SELECT fahrten.id,fahrten.datum, fahrten.startort, fahrten.zielort, fahrten.zweck, fahrten.kmabfahrt,fahrten.kmankunft,fahrten.gefahrene_km, fahrten.tankkosten,fahrer.vorname,fahrer.nachname, kfz.kennzeichen FROM `fahrten`left JOIN fahrer on fahrten.fahrer_id=fahrer.id left JOIN kfz on fahrten.kfz_id=kfz.id WHERE fahrer_id=:fahrer_id');
...
echo " <form method='post'><input type='hidden' name='id' value='".$_POST['driver']."'><input type=\"submit\" name='delete_fahrer_fahrten' value=\"Löschen Alle Fahrten\"></form>";
...
//delete all fahrten
if(isset($_POST['delete_fahrer_fahrten'])) {
    echo "<form method='post'>";
    echo "<p>Sind Sie sicher, dass Sie diese Fahrten löschen möchten?</p>";
    echo "<input type='hidden' name='id' value='".$_POST['id']."'>";
    echo "<input type='submit' name='confirm_delete_fahrten' value='Ja'>&nbsp";
    echo "<input type='submit' name='cancel_delete_fahrtren' value='Nein'>";
    echo "</form>";
}
...
if(isset($_POST['confirm_delete_fahrten'])) {
  ...
  $id = $_POST['id'];
    $sql3 = "DELETE FROM fahrten WHERE fahrer_id = '$id'";
    if(mysqli_query($verbindung, $sql3)) {
       
            echo "Alle fahrten für diesen Fahrer erfolgreich gelöscht";
        }
     else {
        echo "Fehler beim Löschen des Datensatzes: " . mysqli_error($verbindung);
	}
