...
$stmt = $pdo->prepare('SELECT * FROM `fahrten`INNER JOIN fahrer on fahrten.fahrer_id=fahrer.id INNER JOIN kfz on fahrten.kfz_id=kfz.id WHERE fahrer_id=:fahrer_id');
...
echo " <form method='post' action=\"export.php\"><input type='hidden' name=\"fahrer_id\" value=\"$fahrer_id\"><input type=\"submit\" name='Exportieren_fahrer' value=\"Exportieren\"></form>&nbsp";
